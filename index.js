...
// Задача 3: Напишіть функцію, яка перевіряє, чи є задане число
//простим(ділиться без остачі тільки на себе та на 1).
...
// Задача 4: Напишіть функцію, яка перевіряє, чи є задане число
//додатним.Використовуйте умовний оператор для перевірки числа.

function isPositiveNumber(num) {
  // Перевіряємо, чи число num більше 0
  // Якщо так, то num є позитивним, тому повертаємо true
  // В іншому випадку, якщо num менше або дорівнює 0, то num не є позитивним, тому
  //повертаємо false
...
function isEmptyString(str) {
  ...
  // В іншому випадку, якщо str містить хоча б один символ, то str не є порожнім рядком,
...
//і перевіряє його тип даних. Функція повинна використовувати оператор typeof
//для визначення типу значення.
// Виведіть в консоль повідомлення з результатом перевірки типу, наприклад,
// Значення "hello" має тип "string"
...
// Задача 7: Напишіть функцію findMax, яка приймає два числа num1 і num2 і 
//повертає більше з них.Використовуйте умовний оператор if та оператор порівняння 
//для визначення більшого числа.

function findMax(num1, num2) {
  // Порівнюємо num1 та num2
  // Якщо num1 більше num2, то num1 є максимальним, тому повертаємо num1
  // В іншому випадку (якщо num2 більше або дорівнює num1), то num2 є максимальним,
  // тому повертаємо num2
  if (num1 > num2) {
   return num1;
 } else {
    return num2;
...
// Задача 8: Напишіть функцію getUserStatus, яка приймає вік користувача age і 
// повертає рядок, що вказує на його статус. Якщо вік менше 18 років, функція повертає 
//"Неповнолітній".Якщо вік від 18 до 65 років, функція повертає "Дорослий".
...
// Задача 9: Напишіть функцію getDayOfWeek, яка приймає номер дня тижня 
// dayNumber(1 - 7) і повертає рядок, що відповідає цьому дню тижня.
...
function getDayOfWeek(dayNumber) {
  ...
  // В кожному випадку (case) перевіряємо значення dayNumber і присвоюємо змінній day
...
// і повертає рядок, що відповідає типу цієї змінної. Використовуйте тернарний 
  ...
  const message =
    type === 'number' ? "Це число" :
      type === 'string' ? "Це рядок" :
        type === 'boolean' ? "Це булеве значення" : "Це інший тип";
...
// Задача 11: Напишіть рекурсивну функцію, яка обчислює суму чисел від 1 до 
//заданого числа. Факторіал числа - це добуток всіх цілих чисел від 1 до цього числа.
...
function makeAdder(x) {
  ...
  return function newFunction(y) {
    return x + y;
  };
}
const getAdder = makeAdder(5);
// Створюємо змінну getAdder якій присвоємо результат виконання функції makeAdder 
//з аргументом 5
...
// Задача 13: Задача: створити стрілкову функцію, яка приймає два числа і повертає їх 
//добуток.
let multiply = (x, y) => {
...
function memoizedArea() {
  ...
  // Якщо сторона відрізняється від попередньої, обчислюємо площу та зберігаємо результат та 
  // аргумент для майбутнього використання
